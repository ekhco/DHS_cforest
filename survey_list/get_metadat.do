...
log using "get_metadat", t replace
local start = "$S_TIME"
		...
		* ----------------------------------------------------------------------
		...
		if !_rc {
			...
			if !_rc {
				...
				local prev_ = `hiv_pos' / (`hiv_neg' + `hiv_pos')
				local mrg = `mrg' + 1
				
				// -----------------------------------
				// copy the metadata to a folder
				use "meta_data", clear
				saveold "D:\EricChow\DHS_live_abstract\metadat\`filename'_metadat", v(12) replace
		...
		di  "  `"`flattened_file'"'  `hiv_pos' `hiv_neg' `prev_'"
